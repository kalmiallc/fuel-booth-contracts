...
*/
    ...
    // .try_read() returns -> Option<T> : Ok(_) or Err(_). Errors must be handled in Web2
    // .unwrap() returns -> PlayerProfile
   // #[storage(read)]fn player_profile_id(seq_id: u64) -> Option<PlayerProfile> 
    //{
...
//#[storage(read)]
    //fn player_email_safe<E>(seq_id: u64, error_message: E) -> Result<String, E>
    //{
    //    let res: Result<String, E> = storage.players_emails.get(seq_id).try_read;
    //    res;
   //} 
   ...
   // fn player_email_safe<E>(seq_id: u64, error_message: E) -> Result<String, E>
    //{     
     //   storage.players_emails.get(seq_id).read_slice()

       // Result::Ok(error_message)
        //Result::Err(error_message)
        ////match storage.players_emails.get(seq_id).read_slice().unwrap(){
        ////    Ok(_) => { storage.players_emails.get(seq_id).read_slice().unwrap() }
        // //   Err(_) => { String::from_ascii_str("No user at provided ID") }
        ////}  
    // }


   // fn READ_StorageString___get_player_email_by_id(seq_id: u64) -> String  {
        // Method 1: Access the string directly.
        //let stored_email: String = storage.players_emails.get(seq_id).read_slice().unwrap()
        // Method 2: First get the storage key and then access the value.
        // let storage_key: StorageKey<StorageString> = storage.players_emails.get(seq_id);
